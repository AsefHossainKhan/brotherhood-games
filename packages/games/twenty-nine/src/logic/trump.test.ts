...
describe("shouldCancelForHiddenTrump", () => {
  ...
  it("always cancels an unrevealed seventh-card trump (card was set aside)", () => {
    expect(shouldCancelForHiddenTrump("seventh-card", false, 4, 8)).toBe(true);
  });

  it("does not cancel a suit trump while tricks remain", () => {
    expect(shouldCancelForHiddenTrump("suit", false, 4, 8)).toBe(false);
  });
});
